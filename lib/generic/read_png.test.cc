...
void check_matrix(Eigen::MatrixXi expected, Eigen::MatrixXi received) {
    ...
    for(int i = 0; i < expected.rows(); i += 1){
        for(int j = 0; j < expected.cols(); j += 1) {
            std::stringstream ss;
            ss << "i: " << i << " j: " << j << " expected: " << expected(i,j) << " pic: " << received(i,j) << "\n";
...
BOOST_AUTO_TEST_CASE( read_png_grey1x1 ) {
    ...
    const Eigen::MatrixXi pic = MouseTrack::read_png("../lib/generic/read_png.test/grey1x1.png");
    Eigen::MatrixXi expected(1,1);
    expected(0,0) = 128;
...
BOOST_AUTO_TEST_CASE( read_png_grey2x2 ) {
    // Note: make sure to run ./lib/tests to pass this test (from the build directory which in turn is located in the project root)
    const Eigen::MatrixXi pic = MouseTrack::read_png("../lib/generic/read_png.test/grey2x2.png");
...
BOOST_AUTO_TEST_CASE( read_png_grey2x3 ) {
    // Note: make sure to run ./lib/tests to pass this test (from the build directory which in turn is located in the project root)
    const Eigen::MatrixXi pic = MouseTrack::read_png("../lib/generic/read_png.test/grey2x3.png");
    Eigen::MatrixXi expected(2,3);
    expected(0,0) = 26;
    expected(1,0) = 102;
    expected(0,1) = 51;
    expected(1,1) = 128;
