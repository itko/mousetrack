...
if __name__ == "__main__":
    ...
    for file in csv_files:
        ...
        if OLD_COG_READ_TYPE:
            ...
            if file[:13] == 'cluster_cogs_' and file[-4:] == '.csv':
                frame_indices.append(int(file[13:-4]))
    ...
    for frame in frame_indices:
        ...
        if DRAW_CONTROLPOINTS or DRAW_ORIENTATION:
            cpoints = read_controlpoints(os.path.join(csv_dir,"cluster_cogs_" + str(frame) + ".csv"))
            
        if DRAW_COG:
            if OLD_COG_READ_TYPE:
                name = "controlPoints_" + str(frame) + ".csv"
            else:
        ...
        for stream in range(N_STREAMS):
            ...
            if DRAW_CONTROLPOINTS:    
                for cp4 in cpoints: 
                    ...
                    cv2.circle(pic,(cp2[0],cp2[1]),3,(255,0,0),-1)
