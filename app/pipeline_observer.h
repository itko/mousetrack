...
/// Most methods are passed a shared pointer with the output of a pipeline step for a frame.
...
///   Usually you only want to copy the shared pointer and enqueue a job in your run loop
...
/// Guarantees:
///
/// - No concurrent calls: Only one method is called at a time by only one thread.
/// - Sequential FrameIndexes: It is guaranteed, that calls with the same frame index
